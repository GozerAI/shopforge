"""Extended LicenseGate tests for Shopforge — tier restrictions and upgrade paths."""
...
from shopforge.licensing import LicenseGate, PRICING_URL, _FEATURE_TIER_MAP
...
class TestProTierFeatures:
    ...
    def test_advanced_allowed(self):
        ...
        gate._client = _mock_client(features=["std.shopforge.advanced"])
        assert gate.check_feature("std.shopforge.advanced") is True
    ...
    def test_gate_passes_for_advanced(self):
        gate = _make_gate(key="PRO-KEY")
        gate._client = _mock_client(features=["std.shopforge.advanced"])
        gate.gate("std.shopforge.advanced")  # should not raise
...
class TestEnterpriseTierFullAccess:
    ...
    def test_advanced_allowed(self):
        ...
        gate._client = _mock_client(
            features=["std.shopforge.advanced", "std.shopforge.enterprise"]
        )
        assert gate.check_feature("std.shopforge.advanced") is True
    ...
    def test_enterprise_allowed(self):
        ...
        assert gate.check_feature("std.shopforge.enterprise") is True
...
class TestUpgradePath:
    ...
    def test_advanced_tier_is_pro(self):
        _, tier = _FEATURE_TIER_MAP["std.shopforge.advanced"]
        assert tier == "Pro"

    def test_enterprise_tier_is_growth(self):
        _, tier = _FEATURE_TIER_MAP["std.shopforge.enterprise"]
        assert tier == "Growth"

    def test_custom_label_overrides_default(self):
        gate = _make_gate(key="")
        with pytest.raises(PermissionError, match="Custom Label"):
            gate.gate("std.shopforge.advanced", label="Custom Label")

    def test_cache_refreshes_after_upgrade(self):
        """Simulate upgrade: first call returns Pro features, after TTL
        returns Pro+Enterprise."""
        gate = _make_gate(key="UPGRADE-KEY", cache_ttl=1)
        pro_client = _mock_client(features=["std.shopforge.advanced"])
        gate._client = pro_client

        assert gate.check_feature("std.shopforge.advanced") is True
        assert gate.check_feature("std.shopforge.enterprise") is False

        # Expire cache and swap to enterprise-level features
